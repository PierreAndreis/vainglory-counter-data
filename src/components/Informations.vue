    ...
    <section id="hero-data-holder" class="responsive data-holder">
      <section id="v-for-object" class="columns">
        <aside class="menu column is-one-third">
          <ul class="menu-list">
                    <span v-for="value, key in store.heroesData.data.output">
                      ...
                      <a class="is-active" v-if="key == store.heroesData.data.currentHero">{{ key }}</a>
                      <a v-if="key != store.heroesData.data.currentHero">{{ key }}</a>
                      </span>
                        <!--If hero is path specific-->
                      <span v-if="key.split(' ') != key">
                      <a class="is-active" v-if="key == store.heroesData.data.currentHero">{{ key.split(' ')[0] }}
                      ...
                      <a v-if="key != store.heroesData.data.currentHero">{{ key.split(' ')[0] }}
                      <span class="tag is-danger" v-if="key.split(' ')[1] == 'Weapon'">WP</span>
                      <span class="tag is-info" v-if="key.split(' ')[1] == 'Crystal'">CP</span>
                      <span class="tag is-warning" v-if="key.split(' ')[1] == 'Utility'">UTIL</span>
                      </a>
        ...
        <section id="table-content" class="column is-one-half">
          <h2 class="column title">{{ store.heroesData.data.currentHero }} data</h2>
        ...
        {{ store.heroesData.data.ignsLeft }} igns queued
          ...
          <table class="table column">
            ...
            <tr v-for="value, key in store.heroesData.data.output[store.heroesData.data.currentHero]">
              <td class="ign-cell">
                {{key}}
                <span class="tag is-success" v-if="value.winrate >= 54" style="text-transform: uppercase;">LOSES TO {{ store.heroesData.data.currentHero }}</span>
                <span class="tag is-danger" v-if="value.winrate <= 46" style="text-transform: uppercase;">COUNTERS {{ store.heroesData.data.currentHero }}</span>
                <span class="tag is-warning" v-if="key == getStrongestCounter(store.heroesData.data.currentHero)"
